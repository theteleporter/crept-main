...
export const EpisodeCard: React.FC<EpisodeCardProps> = ({ episode }) => {
  ...
  const { title, episodeNumber, description, runningTime, videoQuality } =
    ...
    const handleDownload = async (quality: string, url: string) => {
      ...
      const filename = `${title}.P0${episode.season}.${episode.title}.${quality}.WEBDL.x264.Crept.Studio.mkv`;
      const downloadUrl = `/api/download-video?blobUrl=${encodeURIComponent(url)}&filename=${encodeURIComponent(filename)}`;
